#!/bin/bash
# Startup script to run the orchestrator agent in LLM mode locally

echo "Starting Ad Campaign Orchestrator Agent (LLM Mode)..."
...
if [ -z "$GEMINI_API_KEY" ]; then
    ...
    echo "LLM features will not work without a Gemini API key."
    echo "Please set GEMINI_API_KEY in .env file or as environment variable."
    echo ""
    read -p "Continue anyway? (y/N) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
echo ""
echo "Starting Orchestrator Agent (LLM Mode) on port 8000..."
echo "Using orchestrator service: llm_service.py"
echo ""
...
cd "$(dirname "$0")"
python -m app.orchestrator.llm_service > logs/orchestrator_llm.log 2>&1 &
ORCHESTRATOR_PID=$!

# Save PID
echo $ORCHESTRATOR_PID > logs/orchestrator_llm.pid
...
if ps -p $ORCHESTRATOR_PID > /dev/null 2>&1; then
    echo "✅ Orchestrator Agent (LLM Mode) started successfully!"
    echo ""
    echo "Orchestrator Agent Information:"
    echo "  Mode: LLM-Enhanced (Natural Language Processing)"
    ...
    echo ""
    echo "Key Features:"
    echo "  • Natural language intent parsing"
    echo "  • Intelligent error handling"
    echo "  • Human-readable summaries"
    echo ""
    echo "Endpoints:"
    echo "  • POST /create_campaign_nl - Create campaign from natural language"
    echo "  • POST /create_campaign - Create campaign with structured input"
    echo "  • GET /services/status - Check all service statuses"
    echo ""
    echo "Service URLs:"
    ...
    echo "Logs are available in: logs/orchestrator_llm.log"
else
    echo "❌ Failed to start Orchestrator Agent"
    echo "Check logs/orchestrator_llm.log for details"
    exit 1
fi
